  ...
  return (
    <div className="p-1 flex flex-col gap-4 items-center">
      <div className="flex items-center gap-2 w-full">
        ...
        >
          <span className="text-xs text-slate-300 truncate flex-1 text-left">{translations.colorWheel}</span>
        </Button>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          <span className="text-xs text-slate-300 truncate flex-1 text-left">{translations.colorPalette}</span>
        </Button>
      </div>
      {colorMode === "wheel" && (
        <>
          ...
          >
            {selectedColor === "transparent" && <div className="absolute inset-0 checkerboard-bg opacity-50" />}
            <div className="absolute inset-0" style={{ backgroundColor: selectedColor }} />
            <span className="relative text-xs" style={{ color: getContrastingTextColor(selectedColor) }}>
              {selectedColor}
            </span>
          </div>
          <Colorful
