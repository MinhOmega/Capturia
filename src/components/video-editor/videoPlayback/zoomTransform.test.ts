...
describe('computeFocusFromTransform', () => {
  ...
  it('falls back to the centre for an invalid scale', () => {
    expect(computeFocusFromTransform({ stageSize, baseMask, zoomScale: 0, x: 10, y: 10 })).toEqual({
      cx: 0.5,
      cy: 0.5,
    })
  })
...
describe('stepMotionBlur', () => {
  ...
  it('blurs the same fraction of the frame in preview and export for one camera path', () => {
    ...
    const pan = (stage: { width: number; height: number }) => {
      ...
      const sample = stepMotionBlur(
        state,
        { scale: 1, x: stage.width * 0.02, y: 0 },
        stage,
        0.5,
        16,
      )
...
describe('applyZoomTransform motion blur', () => {
  ...
  it('zeroes the filter and resets the state when not playing or at amount 0', () => {
    ...
    applyZoomTransform({
      ...
      isPlaying: true,
      motionBlurAmount: 1,
      transformOverride: { scale: 1, x: 40, y: 0 },
      frameTimeMs: 16,
    })
    ...
    applyZoomTransform({
      ...
      isPlaying: false,
      motionBlurAmount: 1,
      transformOverride: { scale: 1, x: 80, y: 0 },
      frameTimeMs: 32,
    })
    ...
    applyZoomTransform({
      ...
      isPlaying: true,
      motionBlurAmount: 0,
      transformOverride: { scale: 1, x: 120, y: 0 },
      frameTimeMs: 48,
    })
    applyZoomTransform({
      ...common,
      isPlaying: true,
      motionBlurAmount: 0,
      transformOverride: { scale: 1, x: 160, y: 0 },
      frameTimeMs: 64,
    })
    expect(filter.velocity).toEqual({ x: 0, y: 0 })
  })
