...
describe("buildEditorMenuModel", () => {
  ...
  it("shows Ctrl-based shortcut hints on non-mac", () => {
    ...
    const shortcuts = Object.fromEntries(model.flatMap((m) => m.items).map((i) => [i.id, i.shortcut]));
  ...
  it("reflects the panel visibility as checked state on the View toggles", () => {
    const view = buildEditorMenuModel(makeProps({ timelineVisible: true, settingsVisible: false })).find(
      (m) => m.id === "view",
    );
