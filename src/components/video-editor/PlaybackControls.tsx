    ...
    <div className="flex items-center gap-2 px-1 py-0.5 rounded-full bg-black/60 backdrop-blur-md border border-white/10 shadow-xl transition-all duration-300 hover:bg-black/70 hover:border-white/20">
      ...
      {zoomSlider && (
        <div
          ...
          title={t('common.playback.timelineZoom')}
        >
          ...
          {SPEED_OPTIONS.map((speed) => (
            <DropdownMenuItem
              ...
              className={cn(
                ...
                speed === previewPlaybackRate ? 'text-[#34B27B] font-semibold' : 'text-slate-300',
              )}
            >
              {speed}x
            </DropdownMenuItem>
      ...
      {onToggleFullscreen && (
        <Button
          ...
          aria-label={
            isFullscreen ? t('common.playback.exitFullscreen') : t('common.playback.fullscreen')
          }
        >
