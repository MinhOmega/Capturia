...
const videoAudioContextCache = new WeakMap<
  HTMLVideoElement,
  {
  ...
  (
    ...
    const ensurePreviewAudioGraph = useCallback(() => {
      ...
      try {
        ...
        console.warn(
          'Failed to initialize preview audio processing graph; falling back to HTML media volume.',
          error,
        )
        audioGraphFailedRef.current = true
        return false
    ...
    const normalizedSubtitleCues = useMemo(
      () => normalizeSubtitleCues(subtitleCues),
      [subtitleCues],
    )
    ...
    useImperativeHandle(ref, () => ({
      ...
      play: async () => {
        ...
        console.log(
          '[VideoPlayback] play() called, readyState:',
          vid.readyState,
          'networkState:',
          vid.networkState,
          'src:',
          vid.currentSrc?.slice(-40),
          'duration:',
          vid.duration,
          'paused:',
          vid.paused,
    ...
    const renderCursorOverlay = useCallback(
      (timeMs: number) => {
        ...
        if (
          layout.stageSize.width <= 0 ||
          layout.stageSize.height <= 0 ||
          layout.maskRect.width <= 0 ||
          layout.maskRect.height <= 0
        ) {
          return
        }
        ...
        probeVideo.addEventListener('loadedmetadata', () => {
          const onSeeked = () => {
            ...
            if (actualEnd > 0 && actualEnd < reportedDuration * 0.8) {
              console.warn(
                '[VideoPlayback] WebM duration correction: reported',
                reportedDuration,
                's → actual',
                actualEnd,
                's',
