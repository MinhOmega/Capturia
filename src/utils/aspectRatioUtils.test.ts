...
describe('getNativeAspectRatioValue', () => {
  ...
  it('falls back when crop dimensions are non-positive or non-finite', () => {
    expect(getNativeAspectRatioValue(1920, 1080, { x: 0, y: 0, width: 0, height: 1 })).toBe(FALLBACK_RATIO);
    expect(getNativeAspectRatioValue(1920, 1080, { x: 0, y: 0, width: 1, height: -1 })).toBe(FALLBACK_RATIO);
    expect(
      getNativeAspectRatioValue(1920, 1080, { x: 0, y: 0, width: Number.POSITIVE_INFINITY, height: 1 }),
    ).toBe(FALLBACK_RATIO);
  });
});
