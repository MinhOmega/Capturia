...
describe('getAnnotationFontShorthand', () => {
  it('builds the same shorthand the canvas uses, scaled for export', () => {
    const style = { ...DEFAULT_ANNOTATION_STYLE, fontFamily: '"Fira Code", monospace', fontSize: 20, fontStyle: 'italic' as const };
  ...
  it('loads each distinct face used by text annotations once', async () => {
    ...
    await preloadAnnotationFonts(
      [
        ...
        annotation({ id: 'c', style: { ...DEFAULT_ANNOTATION_STYLE, fontFamily: 'Lora, Georgia, serif', fontWeight: 'normal' } }),
