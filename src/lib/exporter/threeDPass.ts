...
export function transformPoint(m: Float32Array, x: number, y: number, z: number): [number, number, number, number] {
  return [
...
export function createThreeDPass(width: number, height: number): ThreeDPass {
  ...
  const apply = (srcCanvas: HTMLCanvasElement | OffscreenCanvas, rot: Rotation3D): HTMLCanvasElement => {
