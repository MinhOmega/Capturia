...
describe('buildExportDiagnosticMessage', () => {
  ...
  it('omits missing optional fields but always reports source and encoder', () => {
    ...
    expect(message.split('\n')).toEqual(['GIF export failed', 'Source: unknown', 'VideoEncoder: unavailable']);
  });
  ...
  it('uses injected labels for localisation', () => {
    ...
    expect(message.split('\n')).toEqual(['Video 导出失败', '原因: x', '来源: a.mp4', '视频编码器: 可用']);
  });
});

describe('buildSaveDiagnosticMessage', () => {
  it('includes the reason on a second line when given', () => {
    expect(buildSaveDiagnosticMessage('GIF', 'EACCES')).toBe('GIF export save failed\nReason: EACCES');
  });
  ...
  it('honours injected labels', () => {
    expect(buildSaveDiagnosticMessage('Video', 'disk full', { saveFailed: 'Could not save {{format}}', reason: 'Why' })).toBe(
