...
const { VideoExporter, buildAudioSegmentSampleBudget, buildVideoFrameCountsForTimeline } =
  await import('./videoExporter')
...
function makeHarness(opts: HarnessOptions) {
  ...
  exporter.sourceAudioEditRegions = normalizeAudioEditRegions(
    opts.audioEditRegions,
    opts.durationMs,
  )
  ...
  if (opts.decodePath === 'webcodecs') {
    ...
    totalFrames = computeExportMetrics(
    ...
    timeline = segmentsToSpeedTimeline(
      opts.segments,
      trimRegions,
      opts.durationMs / 1000,
      opts.playbackSpeed,
...
describe('buildVideoFrameCountsForTimeline', () => {
  ...
  it('assigns seek-path frames by cumulative output time', () => {
    ...
    expect(buildVideoFrameCountsForTimeline(uneven, FPS, totalFrames, 'seek')).toEqual([
      21,
      totalFrames - 21,
    ])
  })
...
describe('VideoExporter exportAudioTrack with speed segments (WSOLA)', () => {
  ...
  for (const decodePath of ['webcodecs', 'seek'] as const) {
    it(`locks the audio length to the video frame count on the ${decodePath} path (1x, 2x, 0.5x + trim)`, async () => {
      ...
      const { exporter, added, timeline, totalFrames } = makeHarness({
        durationMs: 4000,
        segments,
        decodePath,
      })
  ...
  it('applies audio-edit mute regions before stretching', async () => {
    ...
    const { exporter, added } = makeHarness({
      durationMs: 4000,
      segments,
      audioEditRegions,
  ...
  it('pads an underrun with silence and fills a late-starting track with leading silence', async () => {
    ...
    const { exporter, added, totalFrames } = makeHarness({
      durationMs: 2000,
      playbackSpeed: 2,
      decodePath: 'webcodecs',
    })
  ...
  it('downmixes a 5.1 source to stereo on the stretched path', async () => {
    sinkState.buffers = makeSource({
      durationSec: 1,
      channels: 6,
      fill: (i, c) => (c < 2 ? sine440(i) : 0),
    })
    const { exporter, added } = makeHarness({
      durationMs: 1000,
      playbackSpeed: 2,
