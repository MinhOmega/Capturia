...
/**
 * Loads a local recording as a `File` suitable for demuxers that read on
 * demand (e.g. `web-demuxer`, mediabunny `BlobSource`), without ever holding
 ...
 *      `ERR_FS_FILE_TOO_LARGE` for any file above 2 GiB.
 *   2. Even if it could, a multi-GB `ArrayBuffer`/`Blob` in the renderer would
 *      exhaust memory on typical machines.
 *
 * For recordings above a safe threshold we stream the file into an OPFS-backed
 * file in fixed-size chunks and hand back the disk-backed `File` from
 * `getFile()`. Memory stays flat regardless of size.
 *
 * Concurrency: copies are deduplicated per cache entry; concurrent callers of
 * the same recording share one in-flight copy instead of racing two writables
 * on the same OPFS handle. Each caller can pass an `AbortSignal`; the
 * underlying copy is aborted only once every joined caller has aborted.
 * Successful callers take one cache reference each and must call
...
// overhead low, small enough that peak memory stays bounded. Must not exceed
// the main process `MAX_IPC_CHUNK_BYTES` cap (64 MiB).
const COPY_CHUNK_BYTES = 32 * 1024 * 1024

const OPFS_CACHE_DIR = 'capturia-source-cache'
...
/**
 * Reference counts of OPFS cache entries currently read by a live demuxer,
 * keyed by cache-entry name (which equals the returned File's `.name`).
 * Pruning never removes a name with a live reference.
 */
 ...
 * source was never materialized again). Call once at editor startup: nothing
 * is referenced or mid-copy at that point, so everything stale is reclaimed.
 */
...
/**
 * Returns a `File` for a local recording path/URL, streaming large files
 * through OPFS so nothing multi-GB is ever held in memory.
 *
...
export async function materializeLocalSourceFile(
  ...
  // Large recording: stream into OPFS and hand back a disk-backed File. The
  // OPFS entry keeps its cache-key name; demuxers detect the container from content.
...
async function copyToOpfsFile(
  ...
  if (!getDirectory) {
    throw new Error(
      'This recording is too large to load in memory and cannot be exported: ' +
...
async function runCopy(
  ...
  try {
    ...
    // (Retains happen only after success, but keep the guard in case a prior
    // complete copy of the same name is still being read.)
