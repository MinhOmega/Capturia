...
describe('segmentsToSpeedTimeline', () => {
  ...
  it('keeps the non-trimmed spans with the global speed when only trims are set', () => {
    expect(segmentsToSpeedTimeline(undefined, [trim(2000, 4000), trim(8000, 10000)], 10, 1.5)).toEqual([
...
describe('effective duration parity with the seek path', () => {
  const fixtures: { name: string; segments: VideoSegment[] }[] = [
    ...
    { name: 'deleted middle', segments: [seg(0, 2000), seg(2000, 5000, 1, true), seg(5000, 10000)] },
    { name: 'mixed speeds', segments: [seg(0, 2000, 0.5), seg(2000, 6000, 2), seg(6000, 10000, 4)] },
    { name: 'deleted + speed', segments: [seg(0, 3000, 1, true), seg(3000, 8000, 1.5), seg(8000, 10000, 1, true)] },
  ];
  ...
  it('matches (total - trimmed) / speed for the legacy trims + global speed model', () => {
    ...
    const plan = buildDecodeTimelinePlan({ trimRegions: trims, playbackSpeed: 2, sourceDurationMs: 10000 });
  ...
  it('never emits fewer frames than the seek path estimate for a multi-segment edit', () => {
    const segments = [seg(0, 1500, 2), seg(1500, 4000), seg(4000, 4500, 1, true), seg(4500, 10000, 0.5)];
...
describe('buildDecodeTimelinePlan', () => {
  ...
  it('turns every deleted segment into a trim and every non-1x span into a speed region', () => {
    ...
    expect(plan.speedRegions).toEqual([{ id: 'decode-speed-1', startMs: 4000, endMs: 7000, speed: 2 }]);
  });
});
