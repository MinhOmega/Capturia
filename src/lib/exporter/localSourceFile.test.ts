...
type ChunkResult = { success: boolean; data?: ArrayBuffer; bytesRead?: number; message?: string };
...
function cacheDir(root: FakeDir): FakeDir | undefined {
	return root.subdirs.get("capturia-source-cache");
}
...
function largeSourceApi(url: string, source: Uint8Array, mtimeMs = 1) {
	return {
		...
		readBinaryFile: vi.fn(),
		readFileChunk: vi.fn(
			async (_url: string, offset: number, length: number): Promise<ChunkResult> => ({
				success: true,
				data: source.slice(offset, offset + length).buffer as ArrayBuffer,
