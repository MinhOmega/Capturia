  ...
  for (const preset of PRESETS) {
    for (const offset of SAMPLE_OFFSETS_MS) {
      it(`${preset} at +${offset}ms applies the same opacity, scale and translation`, async () => {
        ...
        const originX =
          (annotation.position.x / 100) * canvasWidth +
          ((annotation.size.width / 100) * canvasWidth) / 2
        const originY =
          (annotation.position.y / 100) * canvasHeight +
          ((annotation.size.height / 100) * canvasHeight) / 2
        expectArgsClose(translateCalls[0].args, [originX, originY])
        expectArgsClose(translateCalls[1].args, [
          css.translateX * scaleFactor,
          css.translateY * scaleFactor,
        ])
  ...
  it('typewriter reveals the same fraction: CSS clip-path inset vs exported grapheme slice', async () => {
    ...
    for (const offset of SAMPLE_OFFSETS_MS) {
      ...
      if (state.revealProgress >= 1) {
        ...
        const expectedVisible = graphemes
          .slice(0, Math.ceil(graphemes.length * state.revealProgress))
          .join('')
