...
/**
 * 240x240 JPEG copies (a few KB each, generated by
 * `scripts/generate-wallpaper-thumbs.mjs`) used only for the picker grid. The
 * project keeps storing the canonical full-res path in `WALLPAPER_PATHS`; the
 * preview and the exporter still decode the original. Without the thumbs the
 * grid decoded every original (~24 MB) just to paint 36 px swatches.
 */
...
/**
 * Loadable URL of the picker thumbnail for a bundled wallpaper. Accepts the
 * canonical path (or a legacy value that normalizes to one); anything else is
 * not a bundled wallpaper and resolves to `null` so the caller can fall back
 * to the value itself.
 */
export async function resolveWallpaperThumbUrl(value: string): Promise<string | null> {
  const index = WALLPAPER_PATHS.indexOf(normalizeWallpaperValue(value))
  if (index === -1) return null
  return resolveImageWallpaperUrl(WALLPAPER_THUMB_PATHS[index])
}
