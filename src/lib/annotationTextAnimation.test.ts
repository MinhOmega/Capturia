...
describe('annotation text animations', () => {
  ...
  it('reveals typewriter text progressively and fully after the duration', () => {
    ...
    expect(getTextAnimationState(annotation, 500 + TEXT_ANIMATION_DURATION_MS / 2).revealProgress).toBeCloseTo(0.5);
    expect(getTextAnimationState(annotation, 500 + TEXT_ANIMATION_DURATION_MS).revealProgress).toBe(1);
  });
  ...
  it('settles every preset once the animation has finished', () => {
    for (const { value } of TEXT_ANIMATION_OPTIONS) {
      const state = getTextAnimationState({ startMs: 0, style: { textAnimation: value } }, TEXT_ANIMATION_DURATION_MS * 2);
