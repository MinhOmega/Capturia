...
function applyCueDurationConstraints(
  ...
  for (let index = 0; index < next.length; index += 1) {
    ...
    const minDurationForCpsMs = Math.ceil(
      (cueTextLength(cue.text) / Math.max(0.001, options.maxCps)) * 1000,
    )
...
export function buildSubtitleCuesFromWords(
  ...
  const flushBuffer = (): void => {
    ...
    const averageConfidence =
      buffer.reduce((sum, word) => sum + (word.confidence ?? 0.8), 0) / buffer.length
    cues.push({
