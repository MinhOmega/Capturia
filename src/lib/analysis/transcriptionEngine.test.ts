...
function analysisFixture(): VideoAnalysisResult {
  return {
    transcript: {
      locale: 'en-US',
      text: 'native words',
      words: [{ text: 'native', startMs: 0, endMs: 300 }],
      createdAtMs: 1,
    },
    subtitleCues: [],
    roughCutSuggestions: [],
...
function engine(
  id: TranscriptionEngine['id'],
  impl: TranscriptionEngine['transcribe'],
): TranscriptionEngine {
...
function baseOptions(overrides: Partial<CaptionGenerationOptions> = {}): CaptionGenerationOptions {
  return {
    ...
    engines: {
      native: engine('macos-speech', async () => ({
        success: true,
        engine: 'macos-speech',
...
describe('runCaptionGeneration', () => {
  ...
  it('does not fall back for other native failures, nor when Whisper is unavailable, nor for setting=native', async () => {
    const nativeNoSpeech = engine('macos-speech', async () => ({
      success: false,
      code: 'no_speech_detected',
      message: 'silent',
    }))
    const noFallback = await runCaptionGeneration(
      baseOptions({ engines: { native: nativeNoSpeech, whisper: baseOptions().engines.whisper } }),
    )
    ...
    const nativeDenied = engine('macos-speech', async () => ({
      success: false,
    ...
    const unavailable = await runCaptionGeneration(
      baseOptions({
        ...
        engines: { native: nativeDenied, whisper: baseOptions().engines.whisper },
      }),
    )
  ...
  it('reports declined / aborted / failed model downloads without running Whisper', async () => {
    ...
    const failed = await runCaptionGeneration(
      baseOptions({
        setting: 'whisper',
        ensureModel: async () => ({ state: 'failed', message: 'offline' }),
      }),
    )
...
describe('createWhisperWebEngine', () => {
  ...
  it('extracts audio, trims leading silence, pins the language and shifts segments back', async () => {
    ...
    const transcribe = vi.fn(
      async (_s: Float32Array, opts: { language?: string; modelDirUrl: string }) => {
    ...
    const phases: string[] = []
    const result = await whisper.transcribe(
      request({ locale: 'vi', onStatus: (p) => phases.push(p) }),
    )
  ...
  it('fails cleanly when the model dir is unknown or nothing was recognised', async () => {
    ...
    expect(await unavailable.transcribe(request())).toMatchObject({
      success: false,
    ...
    expect(await silent.transcribe(request())).toMatchObject({
      success: false,
      code: 'no_speech_detected',
    })
  })
