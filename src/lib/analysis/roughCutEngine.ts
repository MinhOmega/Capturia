 ...
 */
export function isGapInsideSyntheticPhrase(
  previous: TranscriptWord,
  current: TranscriptWord,
): boolean {
  return (
    Boolean(previous.synthetic && current.synthetic) &&
...
export function generateRoughCutSuggestions(
  ...
  const fillerSet = new Set(
    options.fillerWords.map((word) => normalizeTextToken(word)).filter(Boolean),
  )
