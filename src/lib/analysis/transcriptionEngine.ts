...
export function shouldFallbackToWhisper(result: VideoTranscriptionResult): boolean {
  return (
    !result.success && typeof result.code === 'string' && NATIVE_FALLBACK_CODES.has(result.code)
  )
}
...
export function shiftSegmentsBySeconds(
  result: TranscribeMono16kResult,
  offsetSec: number,
...
export function createWhisperWebEngine(
  deps: Partial<WhisperWebEngineDeps> = {},
): TranscriptionEngine {
  ...
  return {
    ...
    async transcribe(request) {
      ...
      const trimRegions = shiftTrimRegionsMsForCaptionBuffer(
        request.trimRegions ?? [],
        Math.round(trimmed.trimSec * 1000),
      )
...
export function createDefaultCaptionEngines(): {
  native: TranscriptionEngine
