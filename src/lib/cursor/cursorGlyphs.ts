...
export function parseCursorGlyphSvg(rawSvg: string): ParsedCursorGlyph {
  ...
  const viewBoxMatch = /viewBox\s*=\s*"\s*([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s*"/.exec(raw);
  ...
  for (const match of raw.matchAll(PATH_TAG_RE)) {
    ...
    shapes.push({
      ...
      lineCap: attributes['stroke-linecap'] === 'round' ? 'round' : attributes['stroke-linecap'] === 'square' ? 'square' : 'butt',
      lineJoin: attributes['stroke-linejoin'] === 'round' ? 'round' : attributes['stroke-linejoin'] === 'bevel' ? 'bevel' : 'miter',
    });
  }

  return {
    viewBox: {
      width: Number.isFinite(viewBoxWidth) && viewBoxWidth > 0 ? viewBoxWidth : CURSOR_GLYPH_VIEWBOX,
      height: Number.isFinite(viewBoxHeight) && viewBoxHeight > 0 ? viewBoxHeight : CURSOR_GLYPH_VIEWBOX,
    },
    shapes,
  };
}
...
export function resolveCursorGlyphDrawPlan(kind: CursorKind): CursorGlyphDrawPlan | null {
  ...
  const plan: CursorGlyphDrawPlan | null = parsed.shapes.length > 0
    ? {
      kind,
 ...
 */
export function drawCursorGlyphPlan(ctx: CanvasRenderingContext2D, plan: CursorGlyphDrawPlan): void {
  ctx.save();
