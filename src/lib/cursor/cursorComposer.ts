...
const SUPPORTED_MOVEMENT_STYLES: CursorMovementStyle[] = [
  'rapid',
  'quick',
  'default',
  'slow',
...
function normalizeCursorStyle(input?: Partial<CursorStyleConfig>): CursorStyleConfig {
  ...
  return {
    ...
    staticHideDelayMs: Math.max(
      0,
      Math.min(
        8000,
        Math.round(
          toFiniteNumber(merged.staticHideDelayMs, DEFAULT_CURSOR_STYLE.staticHideDelayMs),
        ),
      ),
    ),
    staticHideFadeMs: Math.max(
      40,
      Math.min(
        2400,
        Math.round(toFiniteNumber(merged.staticHideFadeMs, DEFAULT_CURSOR_STYLE.staticHideFadeMs)),
      ),
    ),
    loopCursorPosition: Boolean(merged.loopCursorPosition),
    loopBlendMs: Math.max(
      80,
      Math.min(
        10000,
        Math.round(toFiniteNumber(merged.loopBlendMs, DEFAULT_CURSOR_STYLE.loopBlendMs)),
      ),
    ),
    ...
    timeOffsetMs: Math.max(
      -300,
      Math.min(300, Number.isFinite(merged.timeOffsetMs) ? merged.timeOffsetMs : 0),
    ),
...
function getPreparedCursorTrack(track?: CursorTrack | null): PreparedCursorTrack | null {
  ...
  const firstEventTimeMs =
    eventCount > 0 && Number.isFinite(eventsRef?.[0]?.startMs)
      ? Number(eventsRef?.[0]?.startMs)
      : Number.NaN
  const lastEventTimeMs =
    eventCount > 0 && Number.isFinite(eventsRef?.[eventCount - 1]?.endMs)
...
function resolveStaticVisibilityFactor(
  ...
  const normalizedActivityTimes =
    activityTimes && activityTimes.length > 0 ? activityTimes : buildActivityTimes(samples)
  const lastActivityTime =
    findLastTimestampAtOrBefore(normalizedActivityTimes, timeMs) ?? samples[0].timeMs
  ...
  const fadeProgress = clamp01(
    (inactivityMs - style.staticHideDelayMs) / Math.max(1, style.staticHideFadeMs),
  )
...
export function resolveCursorState(params: CursorResolveParams): CursorResolvedState {
  ...
  const staticVisibilityFactor = fromTrack
    ? resolveStaticVisibilityFactor(
        sortedSamples,
        sampleTimeMs,
        style,
        preparedTrack?.activityTimes,
      )
    : 1
...
export function resolveCursorClipRect(args: {
  ...
  const baseRadius = Number.isFinite(args.maskBorderRadius)
    ? Math.max(0, Number(args.maskBorderRadius))
    : 0
...
export function getCursorMotionBlurPx(args: {
  ...
  const sizeNorm =
    Number.isFinite(args.sizeNorm) && Number(args.sizeNorm) > 0 ? Number(args.sizeNorm) : 1
...
export function drawCompositedCursor(
  ...
  const safeContentScale = Math.max(
    0.1,
    Math.min(8, Number.isFinite(contentScale) ? contentScale : 1),
  )
  ...
  const motionBlurPx = Number.isFinite(options.motionBlurPx)
    ? Math.max(0, Number(options.motionBlurPx))
    : 0
