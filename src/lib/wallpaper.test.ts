...
describe('encodeRelativeAssetPath', () => {
  ...
  it('rejects traversal segments, also when percent-encoded', () => {
    ...
    expect(() => encodeRelativeAssetPath('/wallpapers/%2e%2e/app.asar')).toThrow(
      UnsafeAssetPathError,
    )
    expect(() => encodeRelativeAssetPath('/wallpapers//x.jpg')).toThrow(UnsafeAssetPathError)
  })
...
describe('resolveImageWallpaperUrl', () => {
  ...
  it('wraps getAssetPath failures in BackgroundLoadError with a path-free message', async () => {
    ...
    const err = await resolveImageWallpaperUrl('/wallpapers/wallpaper3.jpg').catch(
      (e: unknown) => e,
    )
    expect(err).toBeInstanceOf(BackgroundLoadError)
...
describe('normalizeWallpaperValue (legacy project migration)', () => {
  it('rewrites packaged file URLs (resources/assets/wallpapers/…)', () => {
    expect(
      normalizeWallpaperValue('file:///opt/Capturia/resources/assets/wallpapers/wallpaper5.jpg'),
    ).toBe('/wallpapers/wallpaper5.jpg')
  })

  it('rewrites the flat packaged layout (resources/wallpapers/…)', () => {
    expect(
      normalizeWallpaperValue('file:///opt/Capturia/resources/wallpapers/wallpaper3.jpg'),
    ).toBe('/wallpapers/wallpaper3.jpg')
  })

  it('rewrites dev-server file URLs (public/wallpapers/…)', () => {
    expect(
      normalizeWallpaperValue('file:///home/me/Capturia/public/wallpapers/wallpaper1.jpg'),
    ).toBe('/wallpapers/wallpaper1.jpg')
    expect(
      normalizeWallpaperValue('file:///home/me/Capturia/public/assets/wallpapers/wallpaper4.jpg'),
  ...
  it('rewrites Windows-style file URLs with a drive letter', () => {
    expect(
      normalizeWallpaperValue(
        'file:///C:/Users/me/AppData/Local/Capturia/resources/assets/wallpapers/wallpaper2.jpg',
      ),
    ).toBe('/wallpapers/wallpaper2.jpg')
    expect(
      normalizeWallpaperValue(
        'file://C:/Program%20Files/Capturia/resources/wallpapers/wallpaper7.jpg',
      ),
    ).toBe('/wallpapers/wallpaper7.jpg')
  })
  ...
  it('falls back to DEFAULT_WALLPAPER when the legacy URL points outside the bundled set', () => {
    expect(
      normalizeWallpaperValue('file:///opt/Capturia/resources/assets/wallpapers/wallpaper99.jpg'),
    ).toBe(DEFAULT_WALLPAPER)
...
describe('isSameBuiltInWallpaper', () => {
  it('matches canonical and legacy forms of the same bundled image', () => {
    expect(isSameBuiltInWallpaper('/wallpapers/wallpaper2.jpg', '/wallpapers/wallpaper2.jpg')).toBe(
      true,
    )
    expect(
      isSameBuiltInWallpaper(
        'file:///opt/Capturia/resources/assets/wallpapers/wallpaper2.jpg',
        '/wallpapers/wallpaper2.jpg',
      ),
    ).toBe(true)
    expect(isSameBuiltInWallpaper('/wallpapers/wallpaper3.jpg', '/wallpapers/wallpaper2.jpg')).toBe(
      false,
    )
