...
export interface CaptionModelApi {
  ...
  getCaptionModelStatus: (
    modelId?: string,
  ) => Promise<{ success: boolean; status?: CaptionModelStatus; message?: string }>
 ...
 */
export async function ensureCaptionModel(
  options: EnsureCaptionModelOptions,
): Promise<EnsureCaptionModelOutcome> {
  ...
  const unsubscribe = options.onProgress
    ? api.onCaptionModelProgress(options.onProgress)
    : () => undefined
  const onAbort = () => {
