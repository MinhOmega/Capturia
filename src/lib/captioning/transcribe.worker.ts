...
async function loadTranscriber(opts: {
  ...
  return withoutNodeVersion(async () => {
    ...
    // works on this model.
    const transcriber = (await pipeline('automatic-speech-recognition', opts.modelId)) as unknown as TranscriberFn;
    return transcriber;
  });
}
...
self.onmessage = async (event: MessageEvent<TranscribeWorkerRequest>) => {
  ...
  try {
    ...
    const { segments, granularity } = await runTranscription(transcriber, samples, trimRegions ?? [], {
      language,
    });
