 ...
 * None of the three helpers watches for that: `pipewire-capture` hands its
 * frames to libavformat and never inspects a write error, and neither the
 * ScreenCaptureKit nor the WGC helper mentions free space either. The file is
 * therefore truncated wherever the writer happened to be, which for the native
...
export function assessRecordingDiskSpace(
	...
	if (availableBytes < RECORDING_DISK_SPACE_BLOCK_BYTES) {
		return { level: "blocked", availableBytes };
	}
