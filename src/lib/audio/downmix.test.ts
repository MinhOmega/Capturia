...
describe('downmixPlanarChannelsForExport', () => {
  ...
  it('averages every channel when exporting mono', () => {
    const planes = [new Float32Array([1, 0]), new Float32Array([0, 1]), new Float32Array([0.5, 0.5])];
  ...
  it('rejects unsupported target channel counts', () => {
    expect(() => downmixPlanarChannelsForExport([new Float32Array(1)], 3)).toThrow(/Unsupported target channel count/);
  });
});
