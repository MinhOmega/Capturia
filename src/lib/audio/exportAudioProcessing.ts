...
export function normalizeExportAudioProcessingConfig(
  ...
  const targetLufs = clamp(
    Number.isFinite(input?.targetLufs)
      ? Number(input?.targetLufs)
      : DEFAULT_AUDIO_PROCESSING.targetLufs,
    -32,
    -8,
  )
  const limiterDb = clamp(
    Number.isFinite(input?.limiterDb)
      ? Number(input?.limiterDb)
      : DEFAULT_AUDIO_PROCESSING.limiterDb,
    -12,
    -0.1,
  )
  const maxBoostDb = clamp(
    Number.isFinite(input?.maxBoostDb)
      ? Number(input?.maxBoostDb)
      : DEFAULT_AUDIO_PROCESSING.maxBoostDb,
    0,
    24,
  )
