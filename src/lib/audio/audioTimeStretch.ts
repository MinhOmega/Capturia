// Pitch-preserving audio time-stretch (WSOLA) used by the export path for
// timeline segments whose playback speed is not 1x.
//
...
// windowed grain along the input at the analysis hop (speed x synthesis hop) and
...
/** Speeds within this distance of 1x are treated as passthrough (no resampling). */
export const TIME_STRETCH_PASSTHROUGH_EPSILON = 1e-3

/** Speeds above this are clamped: the grain hop would outrun any sensible input window. */
export const MAX_TIME_STRETCH_SPEED = 100

/** True when a segment at `speed` can be copied through without time-stretching. */
export function isTimeStretchPassthroughSpeed(speed: number): boolean {
  return Math.abs(speed - 1) < TIME_STRETCH_PASSTHROUGH_EPSILON
}
...
export class WsolaTimeStretcher {
  ...
  constructor(opts: WsolaOptions) {
    ...
    const speed = Math.min(MAX_TIME_STRETCH_SPEED, Math.max(Number.EPSILON, opts.speed))
    this.passthrough = isTimeStretchPassthroughSpeed(speed)
  ...
  push(planar: Float32Array[]): Float32Array[] {
    if (this.passthrough) {
      // Copy so callers may reuse their buffers; output === input for 1x spans.
  ...
  private process(final: boolean): Float32Array[] {
    const emitted: Float32Array[] = Array.from({ length: this.channels }, () => new Float32Array(0))
