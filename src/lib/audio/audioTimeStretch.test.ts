...
import { WsolaTimeStretcher, isTimeStretchPassthroughSpeed } from './audioTimeStretch';
...
describe('WsolaTimeStretcher', () => {
  ...
  it('does not fade in from silence at the start of a segment', () => {
    ...
    let peakStart = 0;
    for (let i = 0; i < Math.min(200, out.length); i++) peakStart = Math.max(peakStart, Math.abs(out[i]));
    expect(peakStart).toBeGreaterThan(0.3);
  });

  it('clamps absurd speeds to the 100x cap instead of outrunning the input', () => {
    // 1s at 1000x would ask for a 960k-sample analysis hop on 48k of input; the
    // cap keeps the hop inside the buffer so output is still produced.
    const input = sine(440, 1.0);
    const [capped] = stretch([input], 1000);
    const [hundred] = stretch([input], 100);
    expect(capped.length).toBe(hundred.length);
    expect(capped.every((s) => Number.isFinite(s))).toBe(true);
  });

  it('treats speeds within 1e-3 of 1x as passthrough', () => {
    expect(isTimeStretchPassthroughSpeed(1)).toBe(true);
    expect(isTimeStretchPassthroughSpeed(1.0005)).toBe(true);
    expect(isTimeStretchPassthroughSpeed(1.002)).toBe(false);
    expect(isTimeStretchPassthroughSpeed(0.5)).toBe(false);
  });
});
