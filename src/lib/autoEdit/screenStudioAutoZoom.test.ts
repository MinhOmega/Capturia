...
describe('screenStudioAutoZoom', () => {
  ...
  it('drafts reach full zoom under the Screen Studio transition window and hold it', () => {
    ...
    for (let index = 0; index < 6; index += 1) {
      samples.push({ timeMs: index * 2_400 + 300, x: 0.2 + index * 0.05, y: 0.4, click: true, visible: true });
    }
    ...
    for (const draft of drafts) {
      ...
      expect(draft.endMs - draft.startMs).toBeGreaterThanOrEqual(AUTO_ZOOM_TUNING.minRegionDurationMs);
