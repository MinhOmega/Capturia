...
function collectCandidates(samples: CursorSample[], events: CursorTrackEvent[]): Candidate[] {
  ...
  if (events.length === 0 || !hasEventClicks) {
    ...
    for (const sample of samples) {
      ...
      if (interactionAnchors.some((timeMs) => Math.abs(timeMs - sample.timeMs) < CLICK_MIN_GAP_MS))
        continue
...
function toDraft(candidate: Candidate, durationMs: number): DraftWithWeight | null {
  ...
  const startAnchor = Number.isFinite(candidate.startHintMs)
    ? Number(candidate.startHintMs)
    : candidate.timeMs
  const endAnchor = Number.isFinite(candidate.endHintMs)
    ? Number(candidate.endHintMs)
    : candidate.timeMs
 ...
 */
function carveAroundSpans(
  drafts: DraftWithWeight[],
  avoidSpans: AutoZoomSpan[],
): DraftWithWeight[] {
