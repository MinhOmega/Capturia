...
describe("useCameraDevices", () => {
  ...
  it("should fall back to first available device when selected device is unplugged", async () => {
    ...
    const cam2Only = [{ kind: "videoinput", deviceId: "cam2", label: "Camera 2", groupId: "group1" }];
    mockEnumerateDevices.mockResolvedValueOnce(cam2Only);

    // Trigger devicechange event via the registered handler
    const devicechangeHandler = (navigator.mediaDevices.addEventListener as ReturnType<typeof vi.fn>).mock
  ...
  it("keeps a persisted selection when that camera is still present", async () => {
