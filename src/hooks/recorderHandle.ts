...
type RecordingStreamApi = Partial<
  Pick<Window["electronAPI"], "openRecordingStream" | "appendRecordingChunk" | "closeRecordingStream">
>;

/**
 * Wrap an already-constructed MediaRecorder, optionally streaming its chunks to disk.
 *
 * With `fileName`, chunks stream to disk through the main process so a long recording
 * never buffers the whole video in the renderer. Chunks are held in memory until the
...
export function createRecorderHandle(recorder: MediaRecorder, fileName?: string): RecorderHandle {
  ...
  const api = (typeof window !== "undefined" ? window.electronAPI : undefined) as RecordingStreamApi | undefined;
  ...
  // The caller may never await the promise (discard path); keep an unhandled
  // rejection from surfacing in that case.
  recordedBlobPromise.catch(() => undefined);
  ...
  async function discard(): Promise<void> {
    memoryChunks.length = 0;
    // Let queued writes settle first so the close does not race a chunk still in flight.
    await openPromise.catch(() => undefined);
    await writeChain;
