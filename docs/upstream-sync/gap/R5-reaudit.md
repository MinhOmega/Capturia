...
| A-5 | Dismiss HUD popovers when the window loses focus (click on the desktop never reaches the renderer; Escape undeliverable) | 54e12706 (`window.addEventListener('blur', closePopovers)`, 8 tests) | MISSING. `LaunchWindow.tsx:1203,1216,1303` Radix `Popover`s, no `blur` listener | reimplement | S | **R5-HUD** (LW) |
| A-6 | Camera unplugged mid-recording: `track.onended` → toast, recording continues | #325 (1f99fcb4), key `editor.recording.cameraDisconnected` | MISSING. No `onended` on the camera track in `useScreenRecorder.ts` (composited canvas would freeze on the last frame) | reimplement | S | **R5-A1** (USR) |
...
| A-9 | Drop `ideal 1280x720` camera constraints (respect native orientation) | b5c105b5 (gap W9) | PRESENT-OLD. `useScreenRecorder.ts:600-601` | copy | S | **R5-A1** (USR) — low value while the overlay is burned in |
...
| A-11 | `disable-features=MacCatapLoopbackAudioForScreenShare` (dev crash when `getDisplayMedia({audio})` on macOS) | v1.7.0 `main.ts:30` | MISSING (only matters with A-1) | copy | S | **R5-A1** (MAIN) |
| A-12 | Notes teleprompter mode (auto-scroll, speed, font size, localized readouts) | 44e16483, e383f9be, 7dba0f94 → `notesTeleprompter.ts` (+177-line test), `NotesToolbar.tsx` | DONE (R5-NOTES batch: `src/lib/notesTeleprompter.ts`, toolbar row, `notesTeleprompter` preference; see `reviews/W5-A12-notes-teleprompter.md`) | copy | M | **R5-NOTES** (NotesWindow/Toolbar only) |
...
| A-14 | `LaunchWindow` reads `getStopRecordingShortcut()` on mount instead of localStorage (W3-c backlog) | Capturia-own | backlog | reimplement | S | **R5-HUD** (LW) |
