...
| A19 | Keep HUD interactive on Linux | `LaunchWindow.tsx` `isLinuxHud` (8246d071) | DONE differently (W5): main decides per platform in `hudWindowsHandlers.ts` — Wayland never ignores input; Linux X11 / Windows ignore without forwarded moves and rely on the cursor poll. | — | — |
...
| A24 | HUD click-through + JS drag + content-fit resize (`hud-overlay-ignore-mouse-events`, `hud-overlay-move-by`, `hud-overlay-set-size`) | `windows.ts`, `LaunchWindow.tsx` `setHudMouseEventsEnabled`, `measureHudSize` | DONE (W5, `reviews/W5-A24-A25b-hud.md`): the three channels live in `electron/ipc/hudWindowsHandlers.ts`, guarded to the HUD's own `webContents`; forwarded mouse moves on macOS only, a main-side cursor poll against the renderer's interactive boxes everywhere else; Wayland opts out of all three. Geometry helpers in `src/hooks/useHudLayout.ts`; placement (bottom-centre anchor + size) persisted in `userData/hud-overlay-placement.json`. | L | Verified by unit tests only; needs the manual smoke in the review note. |
| A25 | Vertical tray layout toggle + tray icon sizes | `LaunchWindow.tsx` `trayLayout`, `userPreferences.ts` (c293786e, 993f693b); `main.ts` `trayIconSize = isMac ? 16 : 24` (33a60fed, d526ab4c); tray click/double-click opens HUD (4655e71c) | DONE. Icon size + tray click in W3-a; vertical tray in W5 (`hudOrientation` in `userPreferences.ts`, toggle in the bar header, `data-hud-orientation` on the bar). | S / L | — |
