...
mod tests {
    ...
    ///
    /// Env-gated comme les tests `CAPTURIA_GOLDEN_*` (pas de fixture mp4 commitée) :
    /// `CAPTURIA_SWEEP_FILE=/path/to.mp4 cargo test decode_at_never_fails_past_end -- --ignored`
    #[test]
    #[ignore]
    fn decode_at_never_fails_past_end() {
        let path = std::env::var("CAPTURIA_SWEEP_FILE").expect("set CAPTURIA_SWEEP_FILE");
