    ...
    pub unsafe fn compose_frame(
        ...
        if let Some(track) = cursor_ref.as_ref() {
            ...
            if let Some(plan) = plan {
                ...
                // Anneau de clic : sa propre passe, AVANT le pointeur — il doit passer
                // dessous, et il n'entre jamais dans la traînée, qui n'appartient qu'au
                // pointeur. Parité `compositor_linux.rs` / `compositor_windows.rs`.
                if let (Some(ring), Some(sprite)) = (
                    ...
                    if let Err(err) = self.draw_cursor_sprite(
                        e,
                        ring.placement,
                        ring.size_px,
                        ring.alpha,
...
mod tests {
    ...
    fn seg_visual_renders_the_four_modes_from_a_real_photo() {
        let (Ok(out_dir), Ok(cam)) = (
            ...
            eprintln!("harnais visuel : CAPTURIA_SEG_VISUAL + CAPTURIA_SEG_CAM absents — sauté");
            return;
        };
        if !crate::segmentation::runtime_available() {
