...
//! CAPTURIA_TEST_MEDIA to a directory holding `screen_colors.mp4` and
...
fn media_dir() -> Option<PathBuf> {
    let dir = PathBuf::from(std::env::var("CAPTURIA_TEST_MEDIA").ok()?);
...
fn mp4_export_frame_count_follows_output_fps() {
    let Some(dir) = media_dir() else {
        eprintln!("skipped: set CAPTURIA_TEST_MEDIA");
        return;
    };
