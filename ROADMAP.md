...
- [ ] **One-click cleanup: voice enhancement** — *Auto-level* shipped in 2.2: one click measures the take's loudness on this device and sets the output gain so speech lands around −16 LUFS, within the ±12 dB the preview and export both honour. Denoise stays open, and the blocker is not the DSP: the scene contract has no clip-level audio source, so a cleaned track has nowhere to live that both preview and export would read.
...
- [x] **Hardware encode on Linux** — H.264 export goes through `h264_vaapi` from the compositor's own dmabuf, with no CPU readback, and a one-frame probe falls back to software when the driver refuses the imported frame (Intel iHD 24.1 does). HEVC and non-VAAPI stacks are still software-encoded.
...
- [ ] **Fix:** video disappears from editor after export — [upstream #8](https://github.com/getopenscreen/openscreen/issues/8) (Linux, Manjaro). Upstream marked it fixed in 1.7.0 and the preview no longer draws through the renderer it happened in, so it is awaiting a Linux confirmation before retirement; the one remaining suspect is a compositor rebuild while the preview is paused.
- [ ] **Fix:** crash after stopping macOS recording — [upstream #21](https://github.com/getopenscreen/openscreen/issues/21) (macOS 26.4.1, Apple Silicon). Crash is in the Electron / Node async fs shutdown path; recording artifacts are written correctly. 2.2 closes the window the crash needs: the editor window is created before the HUD closes, so the window count never reaches zero mid-finalisation, and `before-quit` now waits for the take's manifest, sidecar and remux writes and flushes open recording streams (10 s ceiling). Awaiting a Mac to confirm.
...
- [x] **Feature:** apply copied attributes onto an existing region of the same kind — [upstream #24](https://github.com/getopenscreen/openscreen/issues/24). *Paste attributes* in the right-click menu and `Ctrl/Cmd + Shift + V`: the target keeps its place and span and takes everything else from the clipboard, in one undo step. A text annotation keeps its own words; only its look is pasted.
- [x] **Feature:** click highlight — an opt-in ring under the cursor on every recorded click, drawn by the same cursor pass in preview and export, so it follows zoom and tilt exactly like the pointer. Off by default; Saved looks carry it.
- [x] **Feature:** speed up idle stretches — Auto-enhance → *Speed up idle* lays 3× speed regions over stretches of at least 2.5 s with no cursor movement and no speech. The on-device sibling of *Remove dead air* that compresses instead of cutting; never runs on import, one undo step.
- [x] **Feature:** the export dialog remembers format, quality, frame rate, codec, aspect ratios and GIF options between runs — and only after an export that succeeded.
- [ ] **Feature:** keystroke captions — record key presses (opt-in, per take) and turn them into editable text annotations. Deferred: it needs all three capture helpers and a macOS Input Monitoring prompt, and a recorder that captures more of you than you expect is a trust problem before it is a feature.
...
- **2026-09-13** — 2.2. Ticked hardware encode on Linux: the line was stale, `h264_vaapi` has exported from the compositor's dmabuf with a software fallback since 2.1 while the tier text still said "software-encoded". Re-scoped voice enhancement into the shipped *Auto-level* step and a denoise that stays blocked on a clip-level audio source. Re-read upstream #8 and #21 against the tree: #8's renderer no longer exists here and needs a Linux confirmation before retirement; #21 gets a window-order and quit-flush guard pending a Mac. Shipped paste attributes (upstream #24, now genuinely ticked), click highlight, speed up idle, and an export dialog with a memory; recorded keystroke captions as deferred with the reason. Behind the features, a repo-wide audit: renderer-named paths now spend an approval before reaching ffmpeg, the compositor, the document's asset list or the OS shell; the stop-recording closure no longer freezes the first render's cursor mode; hand edits on the timeline build from the store rather than a stale closure, so a concurrent transcript save is not overwritten; and about 6,000 lines of dead renderer and main-process code plus eleven unused packages are gone.
- **2026-09-12** — 2.1 additions, none of which had a line here: recording an area of the screen, saved looks with a default for new projects, zooms at the moments flagged while recording, real poster frames in the project list, a choice of speech model, a recordings folder of your choosing, and an opt-in pre-release update channel. Hardware encode on Linux stays open, although export no longer dies on Intel Arc: when the VAAPI driver refuses an imported frame, which iHD 24.1 does, the export now falls back to software instead of failing at the first frame.
