...
for (const namespace of namespaces) {
	baseData[namespace] = JSON.parse(fs.readFileSync(path.join(baseDir, `${namespace}.json`), "utf-8"));
}
...
function walk(dir) {
	for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
		...
		if (/\.test\.tsx?$/.test(entry.name) || full.includes(`${path.sep}__tests__${path.sep}`)) continue;
