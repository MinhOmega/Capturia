...
function describeOutcomeFailure(
  outcome: HelperRunOutcome,
  timeoutMs: number,
...
export async function transcribeVideoFile(args: {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    const offsetSegmentWords = offsetWords(
      normalizeTranscriptWords(segmentResult.words),
      segment.startMs,
    )
