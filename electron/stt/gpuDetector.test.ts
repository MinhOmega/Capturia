...
describe("gpuDetector", () => {
	afterEach(() => {
		delete process.env.CAPTURIA_WHISPER_SERVER_EXE;
	});
	...
	it("candidateBinaryPaths prepends env override when set", () => {
		process.env.CAPTURIA_WHISPER_SERVER_EXE = "/custom/path/whisper-stt-server";
	...
	it("candidateBinaryPaths honours CAPTURIA_WHISPER_SERVER_EXE when set", () => {
		process.env.CAPTURIA_WHISPER_SERVER_EXE = "/custom/path/whisper-stt-server";
