...
import { type AboutFacts, COPYRIGHT, formatAboutDetail, usesNativeAboutPanel, WEBSITE_URL } from './about';
...
describe('formatAboutDetail', () => {
  it('lays the runtime, the install and the project out one per line', () => {
    expect(formatAboutDetail(facts())).toBe(
      ['Electron 39.2.0 · Chromium 142.0.7444.52 · Node 22.20.0', 'darwin arm64 · dmg', WEBSITE_URL].join('\n'),
    );
  });
  ...
  it('names the install channel, not just the platform', () => {
    expect(formatAboutDetail(facts({ platform: 'win32', arch: 'x64', channel: 'store' }))).toContain(
      'win32 x64 · store',
    );
    expect(formatAboutDetail(facts({ platform: 'linux', arch: 'x64', channel: 'appimage' }))).toContain(
      'linux x64 · appimage',
    );
  });
});
