...
export const UPDATE_REPO = 'MinhOmega/Capturia'
export const LATEST_RELEASE_API = `https://api.github.com/repos/${UPDATE_REPO}/releases/latest`
const OFFICIAL_RELEASE_PREFIX = `/${UPDATE_REPO}/releases/tag/`
...
export type FetchLatestRelease = (
  url: string,
  init: {
...
/** Strict semver ordering (leading zeros rejected, build metadata ignored). */
...
/**
 * Only `https://github.com/<repo>/releases/tag/<tag>` for the exact tag the API
 * reported is accepted; anything else (download links, other hosts, query or
 * hash) is refused so a tampered payload can never send the user elsewhere.
 */
