...
export function bindingToAccelerator(binding: ShortcutBinding): string {
  ...
  const acceleratorKey = KEY_TO_ACCELERATOR[keyLower] ?? (isSingleCharOrFKey ? keyLower.toUpperCase() : binding.key);
...
function isBinding(value: unknown): value is ShortcutBinding {
  return Boolean(value) && typeof value === 'object' && typeof (value as ShortcutBinding).key === 'string';
}
