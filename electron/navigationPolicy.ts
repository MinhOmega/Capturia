import { normalizeExternalUrl } from './ipc/paths'

/**
 * What a Capturia window may navigate to.
 *
 ...
 * `window.open`, an HTTP redirect — and the only one Capturia wants is a reload
 * of the same document (Vite's HMR full reload, `Cmd+R` in development).
 *
 * The stake is high because the editor window runs with `webSecurity: false`
 * (it has to, to read pixels back from the recording it renders). A page that
 * managed to navigate that window somewhere else would run with no same-origin
 * policy at all and with `local-media://` reachable.
 *
 * The rules, in order:
 *
 ...
 *    so the editor must not be able to reload itself as the recorder HUD;
 ...
 *    outside the app document, `local-media:`, `about:blank`, and any URL
 *    carrying `user:password@`.
 */
 ...
 * spoof takes (`localhost.attacker.test`, `capturia.example.com.evil.test`).
 */
...
export function decideNavigation(input: NavigationDecisionInput): NavigationVerdict {
  ...
  // DevTools drives its own WebContents; Capturia has no policy to apply there.
  ...
  // schemes (`javascript:`, `mailto:`, `data:`, `local-media:`) have an empty
  // host and must not be mistaken for the app's own file:// origin.
...
export function attachNavigationPolicy(
  ...
  // Capturia never opens a second Electron window from page content; an
