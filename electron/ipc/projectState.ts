...
import { type CurrentVideoMetadata, readCursorTrackSidecar, sanitizeVideoMetadata } from './cursorTrack'
...
export function registerProjectStateHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('set-current-video-path', async (_, nextPath: string, metadata?: CurrentVideoMetadata) => {
    ...
    if (normalizedPath && !isReadablePathAllowed(normalizedPath, { recordingsDir: recordingsDir })) {
  ...
  ipcMain.handle('get-current-video-path', async () => {
    ...
    return session.currentVideoPath
      ? { success: true, path: session.currentVideoPath, metadata: session.currentVideoMetadata ?? undefined }
      : { success: false };
  });
