 ...
 * buffers the whole video in the renderer. The file name is the key because it
 * is already exchanged across IPC and is unique per recording.
 */
export class RecordingStreamRegistry {
   ...
   * letting the renderer's in-memory fallback take over.
   */
...
export type RecordingStreamIpcResult = { success: boolean; error?: string }
 ...
 * renderer expects. `resolveRecordingOutputPath` must reject anything that is
...
export function registerRecordingStreamHandlers(
  ...
  ipcMain.handle(
    ...
    async (_, fileName: string, chunk: ArrayBuffer): Promise<RecordingStreamIpcResult> => {
      try {
