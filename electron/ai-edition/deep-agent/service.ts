...
const BASE_SYSTEM_PROMPT = [
	"You are an AI video editor working inside Capturia. The user is editing a recording.",
...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
	...
	removeModifier:
		"Delete a modifier (zoom / speed / annotation / camera-fullscreen / audio) by id; the kind is resolved from the id. Touching rows with the same styling render as one pill and are deleted together, so read removedIds in the result for everything that disappeared. This is how you 'remove'/'delete' one — never neutralise it (span 0, speed 1×), which leaves it in the document. For a trim use removeTrim; for a clip use removeClip.",
	removeClip:
