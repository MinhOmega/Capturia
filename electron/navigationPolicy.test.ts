...
const PACKAGED_EDITOR =
  'file:///Applications/Capturia.app/Contents/dist/index.html?windowType=editor'
...
describe('decideNavigation', () => {
  const cases: ReadonlyArray<{
    ...
    {
      name: 'local-media is a subresource scheme, never a document',
      trusted: PACKAGED_EDITOR,
      target: 'local-media://local/Users/me/Movies/take.mp4',
      expected: { action: 'deny', reason: 'unsupported-scheme' },
    },
    {
    ...
    },
    {
      ...
      trusted: 'https://capturia.example.com/app?windowType=editor',
      target: 'https://capturia.example.com.attacker.test/app?windowType=editor',
      expected: { action: 'deny', reason: 'lookalike-host' },
    },
    {
    ...
    {
      name: 'an https link from a packaged window is handed to the browser',
      trusted: PACKAGED_EDITOR,
      target: 'https://github.com/MinhOmega/Capturia/issues',
      expected: {
        action: 'external',
  ...
  it('compares packaged paths case-insensitively on Windows only', () => {
    const trusted = 'file:///C:/Program%20Files/Capturia/dist/index.html?windowType=editor'
    const target = 'file:///c:/program%20files/capturia/dist/index.html?windowType=editor'
...
describe('attachNavigationPolicy', () => {
  ...
  it('never opens a second Electron window and externalises allowlisted URLs', () => {
    ...
    expect(fake.openWindow('https://github.com/MinhOmega/Capturia')).toEqual({ action: 'deny' })
    expect(spies.openExternal).toHaveBeenCalledWith('https://github.com/MinhOmega/Capturia')
